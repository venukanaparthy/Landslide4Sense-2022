...
#location of augmeted files
AUG_PATH = "/Users/venu4461/Desktop/LandslideDetect/Landslide4Sense-2022/"
#location of index file with augmented file names
AUG_DATA_FILES_PATH = "/Users/venu4461/Desktop/LandslideDetect/Landslide4Sense-2022/dataset/augment.txt"
...
def main():
    ...
    if transform is not None:
        ...
        transform_fn = A.Compose([A.HorizontalFlip(), A.VerticalFlip(), A.RandomBrightnessContrast((0,0.5), (0, 0.5))])
        src_aug_loader = data.DataLoader(
                        LandslideDataSet(args.data_dir, args.train_list, transform=transform_fn, max_iters=None,set='labeled'),
                        batch_size=1, shuffle=False, num_workers=args.num_workers, pin_memory=True)
        

        START_INDEX = 5000        
        with open(AUG_DATA_FILES_PATH, mode = "w+", encoding = 'utf-8') as f:

            for i, train_augmented_data in enumerate(src_aug_loader):
              file_id = START_INDEX
              train_augmented_images, train_augmented_labels, _, name = train_augmented_data
              print(i, file_id, name, train_augmented_images[0].shape, train_augmented_labels[0].shape)
              torch.save(train_augmented_images[0], '{}AugData/img/image_{}'.format(AUG_PATH, file_id))
              torch.save(train_augmented_labels[0], '{}AugData/mask/mask_{}'.format(AUG_PATH, file_id))          
              f.write("AugData/img/image_{}".format(file_id))
              f.write('\n')

              START_INDEX += 1
           
        print("Done with data augementation.")

        sys.exit()

    #src_loader = data.DataLoader(
    #                    LandslideDataSet(args.data_dir, args.train_list, list_path_augment=None, data_dir_augment=None, max_iters=args.num_steps_stop*args.batch_size,set='labeled'),
    #                    batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True)
    src_loader = data.DataLoader(
                        LandslideDataSet(args.data_dir, args.train_list, list_path_augment=AUG_DATA_FILES_PATH, data_dir_augment=AUG_PATH, max_iters=args.num_steps_stop*args.batch_size,set='labeled'),
                        batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True)
